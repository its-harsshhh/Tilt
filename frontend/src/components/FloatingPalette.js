...
export default function FloatingPalette({ screenContext, captureFrameFn }) {
  ...
  // Cleanup guide interval on unmount
  useEffect(() => {
  ...
  // -- Guide mode --
  const captureAndGetFrame = useCallback(() => {
    // Access the video element from the parent window (main tab)
    try {
      const opener = window.opener || window.parent;
      if (opener && opener.document) {
        const video = opener.document.querySelector('[data-testid="screen-share-video"]');
        if (video) {
    ...
    // Fallback: try captureFrameFn prop
    if (captureFrameFn) return captureFrameFn();

    return null;
  }, [captureFrameFn]);

  const startGuide = useCallback(async (task) => {
    if (!task.trim()) return;
    setGuideTask(task.trim());
    setGuideStep(1);
    setGuideSteps([]);
    setGuideActive(true);
    setGuideResult(null);
    ...
    const frame = captureAndGetFrame();
    if (!frame) {
      setError('Could not capture screen. Make sure screen sharing is active.');
      setLoading(false);
    ...
    try {
      const res = await fetch(`${API_URL}/api/guide-step`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          image_base64: frame,
      ...
      // Start auto-capture interval (every 6 seconds)
      if (guideIntervalRef.current) clearInterval(guideIntervalRef.current);
      guideIntervalRef.current = setInterval(() => {
        autoAdvanceGuide(task.trim(), data, 1, []);
      }, 6000);

    } catch (err) { setError(err.message); setGuideActive(false); }
    finally { setLoading(false); }
  }, [captureAndGetFrame]);

  const autoAdvanceGuide = useCallback(async (task, prevResult, prevStep, prevSteps) => {
    const frame = captureAndGetFrame();
    if (!frame) return;

    try {
      const completedSteps = [...prevSteps];
      if (prevResult?.step_summary) {
        completedSteps.push(prevResult.step_summary);
      }
    ...
    } catch (e) { /* silent fail on auto-advance */ }
  }, [captureAndGetFrame]);

  // Update the interval callback when state changes
  useEffect(() => {
    if (guideActive && guideTask && guideResult && !guideResult.is_complete) {
      if (guideIntervalRef.current) clearInterval(guideIntervalRef.current);
      guideIntervalRef.current = setInterval(() => {
        autoAdvanceGuide(guideTask, guideResult, guideStep, guideSteps);
      }, 6000);
    }
    return () => { if (guideIntervalRef.current && (!guideActive || guideResult?.is_complete)) clearInterval(guideIntervalRef.current); };
  }, [guideActive, guideTask, guideResult, guideStep, guideSteps, autoAdvanceGuide]);
  ...
  const drawAnnotation = useCallback((frameDataUrl, region) => {
    ...
    img.onload = () => {
      ...
      if (region.label) {
        ...
        const textX = cx + r + 10;
        const textY = cy + 5;
        ctx.strokeText(region.label, textX, textY);
        ctx.fillText(region.label, textX, textY);
      }
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (mode === 'guide') {
      if (!guideActive) startGuide(input);
      else sendChat(input); // allow chat during guide
    } else if (mode === 'decide') {
      sendDecide(input);
    } else {
      sendChat(input);
    }
  }, [input, mode, guideActive, startGuide, sendChat, sendDecide]);
  ...
  const switchMode = (newMode) => {
    if (newMode !== 'guide' && guideActive) stopGuide();
    setMode(newMode);
    setDecisions(null);
    setActiveIdx(-1);
  };
    ...
    <div data-testid="floating-palette" style={{
      ...
      <div style={{ padding: '8px 12px 4px', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{
          ...
          {[
            { key: 'chat', label: 'Assist' },
            { key: 'guide', label: 'Guide' },
            { key: 'decide', label: 'Decide' },
          ].map(m => (
            <button key={m.key} onClick={() => switchMode(m.key)} data-testid={`mode-${m.key}-btn`}
              style={{
                flex: 1, padding: '5px 0', borderRadius: '6px', border: 'none', cursor: 'pointer',
                background: mode === m.key ? 'rgba(255,255,255,0.1)' : 'transparent',
                color: mode === m.key ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.3)',
        ...
        {hasRealContext && mode !== 'guide' && (
          <div data-testid="pip-screen-context" style={{
            ...
            <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', margin: 0, lineHeight: '1.4', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {screenContext}
            </p>
          </div>
        )}
          ...
          <div style={{ textAlign: 'center', padding: '28px 8px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.15)', lineHeight: '1.6' }}>
              {mode === 'guide'
                ...
                : hasRealContext ? 'I can see your screen.\nAsk me anything.' : 'Ask me to help\nwith anything.'}
            </p>
          </div>
        )}

        {/* Guide mode UI */}
        {mode === 'guide' && guideActive && (
          <div data-testid="guide-view" style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {/* Task label */}
            <div style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              background: 'rgba(99,102,241,0.08)', borderRadius: '8px', padding: '6px 10px',
            }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {guideSteps.map((s, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', textDecoration: 'line-through' }}>{s}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {annotatedImage && (
              <div style={{ borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(129,140,248,0.2)' }}>
                <img src={annotatedImage} alt="Screen with annotation" data-testid="guide-annotated-image"
                  style={{ width: '100%', height: 'auto', display: 'block' }} />
              </div>
            )}

            {/* Current instruction */}
            {guideResult && !guideResult.is_complete && (
              <div style={{
                background: 'rgba(129,140,248,0.08)', border: '1px solid rgba(129,140,248,0.15)',
                borderRadius: '10px', padding: '10px 12px',
              }} data-testid="guide-instruction">
                <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)', margin: 0, lineHeight: '1.5', fontWeight: '500' }}>
                  {guideResult.instruction}
                </p>
                {guideResult.detail && (
                  <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', margin: '4px 0 0', lineHeight: '1.4' }}>
                    {guideResult.detail}
                  </p>
                )}
              </div>
            ...
            {guideResult?.is_complete && (
              <div style={{
                ...
                <p style={{ fontSize: '14px', color: '#4ade80', margin: 0, fontWeight: '600' }}>
                  Task complete!
                </p>
            ...
            {!guideResult?.is_complete && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0 4px' }}>
                <div style={{ display: 'flex', gap: '3px' }}>
                  {[0,1,2].map(i => (
                    <div key={i} style={{
                      width: '4px', height: '4px', borderRadius: '50%', background: 'rgba(129,140,248,0.4)',
                      animation: `bounce 1.5s ease-in-out ${i * 0.2}s infinite`,
                    }} />
                  ))}
                </div>
                <span style={{ fontSize: '9px', color: 'rgba(255,255,255,0.2)' }}>
                  Watching... do the step above, I'll auto-advance
                </span>
              </div>
            )}
          </div>
          ...
          <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            {msg.role === 'user' ? (
              ...
              <div style={{ maxWidth: '95%', width: '100%' }}>
                {msg.decisionType && (
                  <span style={{ fontSize: '9px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', color: tagColors[msg.decisionType], marginBottom: '3px', display: 'block' }}>
                    {msg.decisionType}
                  </span>
                )}
            ...
            {['safe', 'smart', 'bold'].map((type, idx) => {
              ...
              return (
                <button key={type} onClick={() => handleSelectDecision(type)} data-testid={`pip-card-${type}`}
                  ...
                  <span style={{ fontSize: '10px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', color: tagColors[type], flexShrink: 0, width: '40px', textAlign: 'left' }}>
        ...
        {loading && (
          <div data-testid="pip-loading" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 4px' }}>
            ...
            <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.2)' }}>
              {mode === 'guide' ? 'Analyzing screen...' : mode === 'decide' ? 'Generating options...' : 'Thinking...'}
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '6px 12px 10px', flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.05)', background: 'rgba(2,6,23,0.95)' }}>
        <div style={{
          ...
          <input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)}
            placeholder={mode === 'guide' ? (guideActive ? 'Ask a question...' : 'What do you want to do?') : messages.length > 0 ? 'Continue...' : mode === 'decide' ? 'Describe the situation...' : 'Ask me anything...'}
          ...
          {input.trim() && !loading && (
            <button onClick={handleSubmit} data-testid="pip-submit-btn" style={{
              background: mode === 'guide' && !guideActive ? 'rgba(99,102,241,0.3)' : 'rgba(129,140,248,0.2)',
              border: '1px solid rgba(129,140,248,0.15)', borderRadius: '8px', padding: '5px 12px', cursor: 'pointer',
              color: 'rgba(199,210,254,0.8)', fontSize: '11px', fontFamily: "'Inter', sans-serif", fontWeight: '600', whiteSpace: 'nowrap',
            }}>
              {mode === 'guide' && !guideActive ? 'Start' : 'Send'}
            </button>
          )}
        </div>
        {error && <p style={{ color: '#f87171', fontSize: '10px', marginTop: '4px', padding: '0 4px', fontFamily: 'monospace' }} data-testid="pip-error">{error}</p>}
      </div>
